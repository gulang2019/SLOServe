...
class SolverRequestVariables:
    def __init__(self, 
        ...
        self.arrive_time_idx = arrive_time_idx
        
        self.prefill_finish_time = model.addVar(0, n_prefill_slots, vtype = GRB.INTEGER, name = f'prefill_finish_time_{request_id}')
        self.prefill_allocateds = model.addVars(n_prefill_slots, lb = 0, ub = config.B, vtype=GRB.INTEGER, name = f'prefill_allocated_{request_id}')
        self.decode_indicators = model.addMVar((n_prefill_slots + n_decode_slots, config.max_spec_decode + 1), vtype = GRB.BINARY, name = f'indicators_{request_id}')
...
def solve(problem: Problem,
    ...
    keep_requests.Start = [0 for _ in range(len(problem.reqs))]
    m.update()
