...
PromaxScheduler::PromaxScheduler(
...
std::tuple<int, double, Batch> PromaxScheduler::f(
    double t,
...
std::vector<BatchSch> PromaxScheduler::_batch_impl(
    std::vector<Request>& reqs,
...
std::tuple<bool, std::vector<int>, std::vector<BatchSch> > PromaxScheduler::schedule(
        std::vector<Request>& reqs,
        int M,
        double current_time,
        bool verbose
...
double PromaxScheduler::spec_sample(int n) {
    assert(n >= 1);
    return (1 - std::pow(spec_decode_alpha, n)) / (1 - spec_decode_alpha);
}

double PromaxScheduler::batch_to_time(int n, size_t decode_steps) {
    assert(n >= 0);
    double t = 0;
...
int PromaxScheduler::time_to_batch(double t, size_t decode_steps) {
    double bs = 16384;
    for (int i = 0; i < hardware_params.size(); i += 3){
        double k1 = hardware_params[i];
        double k2 = hardware_params[i+1];
