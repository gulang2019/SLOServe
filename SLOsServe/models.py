...
class PagedKVCacheManager:
    ...
    def new(
        ...
        init_tensors: List[torch.Tensor] = None 
    ):
        blocks = []
        if init_num_tokens != 0:
...
class ModelImpl:
    ...
    def __post_init__(self):
        ...
        print(f"Allocate {self.kvcache_memory_in_gb} GB for KVCache of {self.name} on"
    ...
    def capture_model(self, kv_caches: List[List[torch.Tensor]]) -> None:
        ...
        print(f"Graph capturing of {self.name} finished in %.0f secs.", elapsed_time)
    ...
    def forward(
        ...
        use_cuda_graph = (num_prefills == 0 and self.use_cuda_graph)
        if use_cuda_graph:
            graph_bs = _get_graph_batch_size(batch_size)
            assert graph_bs >= batch_size
            executable = self.graph_runners[get_pp_group().rank_in_group][graph_bs]
            num_decode_tokens = graph_bs - num_prefills
            input_ids = torch.concat([input_ids, torch.zeros(size = (graph_bs - batch_size,), 
                                   dtype = input_ids.dtype, 
                                   device = input_ids.device)])
            zero_pads = [0] * (graph_bs - batch_size)
            ones_pads = [1] * (graph_bs - batch_size)
            position_ids += zero_pads
            past_seq_lens += zero_pads
            cur_seq_lens += ones_pads
            seq_lens += ones_pads
            slot_mappings += [_PAD_SLOT_ID] * (graph_bs - batch_size)
            block_tables = self.graph_block_tables[:graph_bs]
        else:
