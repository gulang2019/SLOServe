...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < repeat; ++i) {
        ...
        std::cout << "Iteration " << (i + 1) << "/" << repeat
                  ...
                  << std::endl;
        // print out the first 10 batches
        for (size_t j = 0; j < replay_batches.size() && j < 10; ++j) {
            const auto& batch = replay_batches[j];
            std::cout << "  Batch " << j
                      << ": prefill_bs=" << batch.prefill_bs
                      << " max_decode_size=" << batch.max_decode_size()
                      << " total_batch_size=" << batch.batch_size()
                      << " req_batches_n=" << batch.req_batches.size()
                      << std::endl;
            for (size_t k = 0; k < batch.req_batches.size() && k < 5; ++k) {
                const auto& req_batch = batch.req_batches[k];
                std::cout << "    ReqBatch " << k
                          << ": id=" << req_batch.id
